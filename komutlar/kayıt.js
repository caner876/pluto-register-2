...
module.exports.run = async (client, message, args) => {
    ...
    let tag = 'XD'
    let salvoembed = new Discord.MessageEmbed().setColor("RANDOM").setFooter(`Developer By Astral`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
    let salvoerkekembed = new Discord.MessageEmbed().setColor(0x56aaff).setFooter(`Developer By Astral`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
    let salvokızembed = new Discord.MessageEmbed().setColor(0xff00ff).setFooter(`Developer By Astral`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
    ...
    victim.roles.add(ayar.erkekRol2)
